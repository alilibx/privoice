...
1. **Web version — Privoice Cloud (React + Vite SPA on Convex)** *(in progress)* — **O0 ✅ GO** · **O1 ✅ verified** (auth + meetings CRUD) · **C1 ✅ verified** (document ingestion + RAG) · **C2 ✅ verified** (agentic chat over documents: `@convex-dev/agent` + `@convex-dev/rag`, streamed, tool-calling `searchDocuments`/`searchMeetings`, ChatGPT-like threads + in-chat upload — retired C1's custom vector store for `@convex-dev/rag`). · **C3 ✅ verified (merged)** — state-of-the-art shadcn SaaS redesign: editorial type (self-hosted Fraunces/Hanken/JetBrains Mono), full-page chat, agent activity trace + "thinking", in-chat doc thumbnails, Markdown (GFM) answers, per-user model picker (server-validated allowlist, sendMessage resolves model server-side + fail-closed), fully responsive + desktop-collapsible sidebar/rail. · **C4 ✅ code-complete (branch, gate green; browser smoke pending)** — Retrieval v2: deterministic hybrid pipeline (BM25 `knowledgeChunks`+searchIndex + vector fused via `hybridRank`) → pin/boost → LLM-judge rerank → labeled context pack with `[n]` citations; unified docs+meetings corpus; `searchKnowledge`/`pinpoint` tools; offline eval harness (recall@k/citation-correctness/attachment-golden). **Next web slices:** **meeting capability on web** (audio upload → server-side STT → minutes; eval Cohere Transcribe — also makes `searchMeetings` substantive), **O2** billing/BYOK + OAuth fast-follow. Then **2. Online tier** (O2/O3/O5 — much now built), **3. iOS**, **4. Desktop** — see the Platform build order below. *Follow-ups: `searchMeetings` thin until web meetings carry transcripts; `@ai-sdk/openai` version pin → add npm `overrides`.*
