...
**Now (web):** **C3 — web redesign** shadcn SaaS shell (collapsible sidebar/topbar/theme-toggle/user-menu + react-router) + light/dark/system theming (mirrored from mobile scheme) + agent activity trace (tool-calling steps from message parts) + in-chat document thumbnails (file-type cards, live parse status) + **model picker** (Settings: appearance + cost/quality comparison over a server-validated allowlist — gpt-4o-mini/gemini-2.5-flash/claude-haiku-4.5/claude-sonnet-5/gpt-5.4/gpt-5.5, live OpenRouter pricing, per-user default resolved server-side in sendMessage, fail-closed) — **code complete, all reviews clean, gate green (tsc + 55/55 tests + build); browser smoke pending**. Branch `feat/web-redesign`.
