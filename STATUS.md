...
**Now (web):** **C7 — chat document reader tools ✅ code-complete (gate green; browser smoke pending) on `feat/web-chat-reader-tools`.** Two retrieval primitives modeled on Claude Code's file toolkit: **`readDocument`** (positional, `cat -n`-numbered read of one source by `sourceId` — `startLine`/`maxLines`, 200-line/8 KB caps; fixes "read the first line of each document") and **`grep`** (regex across *all* sources or one via optional `sourceId`, returns `title:line` coordinates the model expands with `readDocument`) — **replacing `pinpoint`** (folded in; keeps its ReDoS + window-merge hardening, now line-numbered). **`listDocuments`** now surfaces each doc's `sourceId` handle + `lineCount`; new `knowledge.corpusForUser`/`lineCountsForUser` queries back it; `ToolTrace` labels updated (grep/readDocument; pinpoint kept legacy for old messages). **`outline` descoped** — ingest parser flattens every format to raw text, so no heading structure is stored. Built inline-TDD (5 tasks). Gate green (tsc + **130/130 tests** + flutter analyze); spec+plan in `docs/superpowers/`. **Next: browser smoke then merge --no-ff.**
**Prev (web):** **C6 — delete chat conversation ✅ code-complete (gate green; browser smoke pending) on `feat/web-delete-chat`.** Ownership-gated `chat.deleteThread` mutation (reuses `authorizeThread`; deletes our `chatThreads` row then `chatAgent.deleteThreadAsync` to wipe the thread's messages/streams; non-owner → generic "Not found"). UI: per-conversation **⋯ kebab menu** in the rail (visible on hover / active / focus) with a destructive **Delete** → **confirm dialog** ("This can't be undone"); deleting the active conversation re-selects the most recent remaining (empty state if none). Gate green (convex codegen + tsc + **118/118 tests** + build); spec+plan in `docs/superpowers/`.
**Earlier (web):** **C5 — chat UX + document de-dup + list-documents ✅ (merged to main; gate green; browser smoke pending).** Three focused slices built via SDD (7 TDD tasks): **(1) chat stick-to-bottom** — a pure `useStickToBottom` hook drives the message list; the user's own send always jumps to the latest (rAF after optimistic echo), the reply follows the stream *only while at bottom*, and a "Jump to latest ↓" pill appears when scrolled up; resets on thread switch. **(2) Exact-duplicate document guard** — client-side SHA-256 (`content-hash.ts`) + optional `contentHash` column; on same-name **and** identical-bytes upload a subtle `DuplicateDialog` asks "Use existing / Upload anyway" (changed-content-same-name uploads normally). Both paths covered: Documents page (Use existing = no-op + toast) and in-chat attach (Use existing = pin the existing doc, no second copy — the "reference documents" path). **(3) `listDocuments` tool** — userId-scoped `internal.documents.listForUser` + fail-closed agent tool + instruction, so "list my documents" enumerates the *full* inventory, not just RAG hits. Gate green (convex codegen + tsc + **114/114 tests** + vite build). Commits on branch; not yet merged. **Next: browser smoke (`npx convex dev` + sign-in) then merge --no-ff.**
**Yet earlier (web):** **C4 — Retrieval v2 ✅ merged to main (gate green + whole-branch reviewed; browser smoke pending).** Deterministic hybrid retrieval pipeline replacing vector-only search: **unified corpus** (documents *and* meetings embedded into RAG, tagged `source`), **BM25 arm** (`knowledgeChunks` + Convex full-text `searchIndex`) fused with vector via the component's `hybridRank`, **attachment/meeting pinning** (validated `pinnedSourceIds`, inclusive boost — multi-doc still works), **LLM-judge rerank** (fail-soft, config-gated), and a **labeled context pack with `[n]` citations** → inline markers + Sources list in chat. Tools: `searchKnowledge` + `pinpoint` (replaced searchDocuments/searchMeetings). Backed by an **offline retrieval eval harness** (recall@k 4/4, citation-correctness, attachment golden, BM25-only load-bearing). Security: `userId` server-side, pins validated as owned, key server-only. Gate green (convex codegen + tsc + 102/102 tests + build). Built via SDD (11 tasks each spec+quality reviewed + whole-branch review that caught/fixed 2 blockers: non-idempotent BM25 mirror; pin dropped by rerank). **Merged to main (`250f709`, --no-ff). Next: browser smoke — `npx convex dev` + `npx convex run meetings:backfill` (one-time, ingests existing meetings) + sign-in.**
**Now:** Full on-device flow (onboarding → record → transcribe → summarize) **working on the Redmi**. Redesign R1–R6 done + **on-device verified**; **reliable model download shipped + verified**. **R1** calm-teal tokens + light/dark/system theme ✅ · **R2** perceived-perf (LLM streaming, reuse, warm-up) ✅ · **R3** onboarding + background download ✅ · **R4** library-first Home ✅ · **R5** Record screen + live scrolling waveform ✅ · **R6** Overview/Transcript meeting screen (auto-generate, checkable action items, AI title, rename, share) ✅ *(verified, Redmi)*. **Testing:** T0 ✅ · T1 🔨 (privacy ✅) · T2 ✅ (CI) · T3 ✅ (Test Lab). **Next major workstream: Web version (Privoice Cloud — React + Vite SPA on Convex), then online tier → iOS → desktop.** (R7 polish + S4 export remain Android-app backlog.) **iOS bring-up done early (2026-07-27): I0 ✅ — the app now builds and runs on the iOS Simulator with the whole on-device flow working (record → STT → minutes), no package changes needed; see the iOS table for I1–I3.** **Trustworthy minutes (2026-07-27) ✅ code-complete, device verification pending:** `SummarizeGate` now refuses to summarize a too-thin transcript (word-count + density thresholds) *before* any LLM call, closing the fabricated-minutes defect found during iOS bring-up; Overview shows a blocked state with the real reason + the transcript + a "Summarize anyway" override, and a blocked Regenerate now surfaces a SnackBar instead of silently no-op'ing. Minutes and action items are now **hand-editable** (full-screen Markdown editor for minutes; add/edit/delete/drag-reorder for action items), each persisted immediately; Regenerate warns ("Replace your edits?") before discarding hand-edits. Home's subtitle now shows **"Transcript only"** for a transcribed-but-not-summarized meeting. 10 of 11 planned tasks done (Task 11 = on-device verification, next).
...
1. **Web version — Privoice Cloud (React + Vite SPA on Convex)** *(in progress)* — **O0 ✅ GO** · **O1 ✅ verified** (auth + meetings CRUD) · **C1 ✅ verified** (document ingestion + RAG) · **C2 ✅ verified** (agentic chat over documents: `@convex-dev/agent` + `@convex-dev/rag`, streamed, tool-calling `searchDocuments`/`searchMeetings`, ChatGPT-like threads + in-chat upload — retired C1's custom vector store for `@convex-dev/rag`). · **C3 ✅ verified (merged)** — state-of-the-art shadcn SaaS redesign: editorial type (self-hosted Fraunces/Hanken/JetBrains Mono), full-page chat, agent activity trace + "thinking", in-chat doc thumbnails, Markdown (GFM) answers, per-user model picker (server-validated allowlist, sendMessage resolves model server-side + fail-closed), fully responsive + desktop-collapsible sidebar/rail. · **C4 ✅ merged (gate green + reviewed; browser smoke pending)** — Retrieval v2: deterministic hybrid pipeline (BM25 `knowledgeChunks`+searchIndex + vector fused via `hybridRank`) → pin/boost → LLM-judge rerank → labeled context pack with `[n]` citations; unified docs+meetings corpus; `searchKnowledge`/`pinpoint` tools; offline eval harness (recall@k/citation-correctness/attachment-golden). **Next web slices:** **(a) browser-smoke C3+C4 live** (sign-in + `meetings:backfill`); **(b) meeting capability on web** (audio upload → server-side STT → minutes; eval Cohere Transcribe — meetings already retrieve on title+notes via C4, transcripts fill the gap), **O2** billing/BYOK + OAuth fast-follow. Then **2. Online tier** (O2/O3/O5 — much now built), **3. iOS**, **4. Desktop** — see the Platform build order below. *C4 follow-ups (deferred, logged in review): per-user→per-request pin scoping; `ingestMeeting` retry-on-failure; `candidates.ts` unit test; `pinpoint` per-line ReDoS budget (length cap already in). General: `@ai-sdk/openai` version pin → add npm `overrides`; web meeting retrieval covers title+notes, transcripts absent until web capture ships.*
...
| — | **Trustworthy minutes** (insufficiency guard + editable minutes/actions) | ✅ *(code-complete; on-device verification pending — Task 11)* | On `feat/trustworthy-minutes`. `SummarizeGate` (`packages/ai`) blocks summarization of a too-thin transcript before any LLM call (fixes a real fabricated-minutes defect from the iOS bring-up); Overview shows the blocked state + reason + transcript + "Summarize anyway"; blocked Regenerate shows a SnackBar instead of a silent no-op. `Meeting.minutesEditedAt` (schema v4) drives a "Replace your edits?" confirm before Regenerate discards hand-edits. Minutes gain a full-screen Markdown editor (save/cancel-with-confirm); action items gain add/edit/delete/drag-reorder (`ActionItemList`), each persisting immediately. Home subtitle gains **"Transcript only"** for a transcribed-but-unsummarized meeting. `transcript_screen.dart` split: `OverviewTab` + `MinutesEditorScreen` + `ActionItemList` extracted. Built via SDD (11 tasks, each spec+quality reviewed); gate green (analyze clean + **151 tests** green) at every task. Spec+plan in `docs/superpowers/`; task ledger in `.superpowers/sdd/2026-07-27-trustworthy-minutes/`. **Next: Task 11 — on-device (Redmi) verification, then merge --no-ff.** |
...
| Web | React + Vite (SPA) | online tier only | **1 (in progress)** | 🔨 O1 auth+CRUD · C1 docs/RAG · C2 agentic chat · C3 shadcn redesign · C4 retrieval v2 · C5 chat-UX+de-dup+list-docs — all ✅ merged · C6 delete-chat · C7 reader-tools ✅ code-complete on branch (C3–C7 browser smoke pending) |
| Online tier (mobile) | Convex + OpenRouter | opt-in online | **2** | ⬜ new |
| iOS | Flutter | on-device | **3** | 🔨 **I0 ✅ runs on iOS Simulator** (full on-device flow verified: record → STT → minutes) · I1–I3 open |
| macOS / Windows / Linux | Flutter (same codebase) | on-device | **4** (macOS first) | ⬜ new |

### iOS (Flutter, on-device) — reuses audio/stt/ai packages unchanged
| ID | Item | Status | Notes |
|----|------|--------|-------|
| I0 | Enable iOS platform + verify sherpa/fllama/record build & run | ✅ *(verified, iPhone 17 Simulator, iOS 26.5, 2026-07-27)* | `flutter create --platforms=ios` scaffold (`com.privoice.mobile`); **deployment target 14.0** (`background_downloader` SPM floor — 13.0 fails Target Integrity); `NSMicrophoneUsageDescription` added. **Zero Dart changes needed in packages** — all prod paths were already cross-platform (`getApplicationSupportDirectory`/`DocumentsDirectory`). fllama's `hook/build.dart` native-assets path builds llama.cpp for iOS w/ Metal (embedded shader lib) out of the box; **first build ≈ 7.5 min**, incremental ≈ 25 s. Fixed: `main.dart` dev sentinels called Android-only `getExternalStorageDirectory()` → new `lib/dev_sentinels.dart` (`devSentinelDir()`). **End-to-end verified on simulator:** model download (parakeet + Llama-3.2-1B from HF) → mic record → STT (`"So is it working?"` transcribed correctly) → streamed minutes + AI title + 4 action items → sqflite persistence across relaunch |
| I1 | Real-device run: signing/provisioning + on-device perf numbers (RTF, RAM, thermals) | ⬜ | Simulator proves correctness, not perf — Metal on sim is host-GPU. Needs an Apple dev account + a physical iPhone |
| I2 | Background recording: `UIBackgroundModes: audio` + interruption handling (calls/Siri/route change) | ⬜ | Android's foreground-service equivalent. Today recording stops when backgrounded |
| I3 | iOS UX/platform pass: Cupertino affordances, share sheet, notification permission prompt, app icon/launch screen | ⬜ | Icons/launch screen are still the Flutter template |
...
| C2 | **Web agentic chat over documents** | ✅ *(verified)* | `@convex-dev/agent` (threads, streaming via DB deltas, tool-loop, OpenRouter `gpt-4o-mini`, `stopWhen` multi-step) + **`@convex-dev/rag`** (namespace=userId) — **retired C1's custom `documentChunks` store**; ingestion keeps parsing + Arabic chunker as rag splitter. Tools: `searchDocuments` (rag.search) + `searchMeetings` (title+notes). ChatGPT-like threads (auto-title from 1st msg), **in-chat upload**, optimistic echo. Per-user thread ownership + tool userId isolation (traced through installed source); `/security-review` clean; convex-test authz + fresh-clone tests + CI. Verified live (general chat + doc-grounded RAG answer) |
| C7 | **Chat document reader tools (readDocument + grep)** | ✅ *(code-complete; browser smoke pending)* | On `feat/web-chat-reader-tools` (not yet merged). **`readDocument`** — positional, `cat -n`-numbered read of one source by `sourceId` (`startLine`/`maxLines`; 200-line/8 KB caps) → fixes "read the first line of each document". **`grep`** replaces `pinpoint` — regex across *all* sources or one via optional `sourceId`, returns `title:line` coordinates to expand with `readDocument`; keeps ReDoS + window-merge hardening, now line-numbered. **`listDocuments`** gains a `sourceId` handle + `lineCount`; new `knowledge.corpusForUser`/`lineCountsForUser` queries; `ToolTrace` labels updated (pinpoint kept legacy). `outline` descoped (parser stores no structure). Built inline-TDD (5 tasks). Gate green (tsc + 130/130 + flutter analyze); spec+plan in `docs/superpowers/`. Browser smoke + merge pending |
| C6 | **Delete chat conversation** | ✅ *(code-complete; browser smoke pending)* | On `feat/web-delete-chat` (not yet merged). Ownership-gated `chat.deleteThread` (reuses `authorizeThread` → delete `chatThreads` row → `chatAgent.deleteThreadAsync`; non-owner → "Not found"). Rail gains a per-row **⋯ kebab menu** (hover/active/focus) with destructive **Delete** → **confirm dialog**; deleting the active thread re-selects the most recent remaining (empty state if none). Gate green (codegen + tsc + 118/118 + build); spec+plan in `docs/superpowers/`. Browser smoke + merge pending |
| C5 | **Web chat UX + doc de-dup + list-documents** | ✅ *(code-complete; browser smoke pending)* | On `feat/web-chat-ux-dedup` (not yet merged). **(1)** Chat stick-to-bottom (`useStickToBottom`): send always jumps to latest, reply follows stream while at bottom, "Jump to latest" pill when scrolled up. **(2)** Exact-duplicate guard: client SHA-256 + optional `contentHash` column → `DuplicateDialog` on same-name+same-bytes (Use existing / Upload anyway); in-chat "Use existing" pins the existing doc (no second copy). **(3)** `listDocuments` tool (fail-closed, `internal.documents.listForUser`) so "list my documents" enumerates the full inventory. **Merged to main (`--no-ff`).** Gate green (codegen + tsc + 114/114 + build); spec+plan in `docs/superpowers/`. Browser smoke pending |
...
- **Trustworthy minutes — nested-list edge-drag limitation (accepted product decision).** `ActionItemList`'s `ReorderableListView` is nested inside `OverviewTab`'s outer scrollable with `shrinkWrap: true`, which zeroes the inner scrollable's `maxScrollExtent`, so Flutter's `EdgeDraggingAutoScroller` (which binds to the inner `CustomScrollView`) has nothing to scroll and never reaches the outer Overview list. Consequence: with more action items than fit on screen, an item cannot be dragged past the visible viewport during an edge drag. Empirically confirmed (30-item probe: outer scroll offset stayed 0.0 while the inner `maxScrollExtent` was 908). **Accepted** — meetings typically produce 3–6 action items, and the fallback (a separate reorder-mode toggle) was judged not worth the complexity. No test locks in the limitation (one would fail the day someone fixes it).
- **Trustworthy minutes — item-only edits don't trigger the Regenerate warning.** Editing action items (without touching minutes) in a previous app session leaves no durable signal, so a later Regenerate in a new session will not show "Replace your edits?" for those item edits — only `minutesEditedAt` (persisted) is checked; there is deliberately no second persisted column for "items edited since generation." Same-session item edits are still safe (compared against the in-memory last-generated list). Accepted as a rare-path tradeoff.
...
- **iOS build:** Xcode 26.5, CocoaPods 1.16.2, iOS deployment target **14.0** (raise, don't lower — `background_downloader` requires ≥14). First `flutter build ios --debug --simulator` takes **~7.5 min** (fllama compiles llama.cpp via native assets); incremental ~25 s. `record_darwin` + `sherpa_onnx_ios` don't support Swift Package Manager yet → CocoaPods path (warning is benign, will break when Flutter drops Pods).
- **Trustworthy minutes — `SummarizeGate` thresholds** (`packages/ai/lib/src/summarize_gate.dart`): `minWords = 30` (word count below this → `tooFewWords`, checked first); `minWordsPerMinute = 20` (words-per-minute below this → `tooSparse`); the density (wpm) rule only applies once `durationMs >= densityFloorMs` (`densityFloorMs = 60_000`, i.e. 60s) — below that floor, or when `durationMs <= 0`, word count alone decides and the density check is skipped (no divide-by-zero). First estimates, not measured; named constants in one file so they're easy to retune, and the blocked state is derived at render time (not persisted) so retuning re-evaluates old meetings correctly.
- **iOS dev sentinels:** `.seed` / `.ai_selftest` live in **app-documents** on iOS (no external storage) — `devSentinelDir()` in `apps/mobile/lib/dev_sentinels.dart`. Reach the container with `xcrun simctl get_app_container booted com.privoice.mobile data`.
...
3. **iOS** (Flutter) enablement — **I0 ✅ done early** (runs + full flow verified on simulator); remaining: **I1** real-device signing/perf → **I2** background audio → **I3** iOS UX/icons
